...
struct GrowthPlanValidator {
    ...
    static func printDump(pChunks: [SkeletonChunk]) {
        ...
        for chunk in pChunks {
            print("List-Chunk{\(chunk.id)} in question, \(chunk.children.count) pieces, \(chunk.flexers.count) flexers.")
        }
        print("===========================================")
    }
    ...
    static func printDump(pNodes: [SkeletonNode]) {
        print("===========================================")
        for node in pNodes {
            print("List-Node{\(node.id)} in question, \(node.children.count) chunks, \(node.countPieces()) pieces, \(node.countFlexers()) flexers.")
        }
        print("===========================================")
    }
    ...
    static func printDump(pSections: [SkeletonSection]) {
        ...
        for section in pSections {
            print("List-Section{\(section.id)} in question, \(section.children.count) nodes, \(section.countChunks()) chunks, \(section.countPieces()) pieces, \(section.countFlexers()) flexers.")
        }
        print("===========================================")
    }
    ...
    static func printDump(pRows: [SkeletonRow]) {
        print("===========================================")
        for row in pRows {
            print("List-Row{\(row.id)} in question, \(row.children.count) sections, \(row.countNodes()) nodes, \(row.countChunks()) chunks, \(row.countPieces()) pieces, \(row.countFlexers()) flexers.")
        }
        print("===========================================")
    }
    ...
    static func printDump(pRow: SkeletonRow) {
        print("===========================================")
        print("Row{\(pRow.id)} in question, \(pRow.children.count) sections, \(pRow.countNodes()) nodes, \(pRow.countChunks()) chunks, \(pRow.countPieces()) pieces, \(pRow.countFlexers()) flexers.")
        for section in pRow.children {
            print("\tSection{\(section.id)} \(section.children.count) nodes (\(section.childrenSize) / \(section.currentSize))")
            for node in section.children {
                print("\t\tNode{\(node.id)} \(node.children.count) chunks (\(node.childrenSize) / \(node.currentSize))")
                for chunk in node.children {
                    print("\t\t\tChunk{\(chunk.id)} with \(chunk.children.count) pieces and \(chunk.flexers.count) flexers. (\(chunk.childrenSize) / \(chunk.currentSize))")
                }
            }
        }
        print("===========================================")
    ...
    static func printDump(pGrowthPlan: GrowthPlan) {
        ...
        print("GrowthPlan-Section{\(section.id)} in question, \(section.children.count) nodes, \(section.countChunks()) chunks, \(section.countPieces()) pieces, \(section.countFlexers()) flexers.")
        print("===========================================")
    ...
    static func printDump(pRowGrowthPlansList: [RowGrowthPlans]) {
        ...
        for rowGrowthPlans in pRowGrowthPlansList {
            ...
            for growPlan in rowGrowthPlans.growthPlans {
                ...
                print("\t\tGrowPlan, amount \(growPlan.amount), section \(section.id), \(section.children.count) nodes (\(section.childrenSize) / \(section.currentSize))")
            }
        }
        print("===========================================")
    }
    ...
    static func checkSections(pRows: [SkeletonRow],
        ...
        for pRow in pRows {
            ...
            let sectionsUnion = union(list1: growthPlanSections, list2: pRow.children)
    ...
    static func checkNodes(pRows: [SkeletonRow],
        ...
        for pRow in pRows {
            ...
            for growthPlan in growthPlans {
                let mergedNodes = union(list1: pNodes, list2: growthPlan.section.children)
    ...
    static func checkChunks(pRows: [SkeletonRow],
        ...
        for pRow in pRows {
            ...
            for growthPlan in growthPlans {
                ...
                for node in growthPlan.section.children {
                    let mergedChunks = union(list1: pChunks, list2: node.children)
            ...
            for growthPlan in growthPlans {
                ...
                for node in growthPlan.section.children {
                    ...
                    for chunk in node.children {
                        ...
                        let mergedPieces = union(list1: pPieces, list2: chunk.children)
