...
struct PulseStepDTests {
    
    @MainActor @Test func test_two_grow_one_simple_rule_few_pulses() {
        
        let nnode = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 10,
                                                                       currentSize: 10,
                                                                       growCount: 5,
                                                                       amount: 3)
        let node = nnode.0
    ...
    @MainActor @Test func grow_2_flexers_1_section_grow_by_1() {
        
        let flexer_a = GenerateFlexers.generate(size: 1)
        let flexer_b = GenerateFlexers.generate(size: 2)
        
        let node_a = GenerateNodes.generate_currentSizeAccumulate(flexers: [flexer_a, flexer_b])
    ...
    @MainActor @Test func grow_2_flexers_1_section_grow_by_1_utility() {
        let node_a = GenerateNodes.generate_currentSizeAccumulate(flexers: [])
    ...
    @MainActor @Test func grow_large_flexer_grow_no_gaps_10_exact_fit() {
        
        var flexerListA = GenerateFlexers.generate_n(10, size: 0)
        flexerListA.append(GenerateFlexers.generate(size: 1))
        
        var flexerListB = GenerateFlexers.generate_n(10, size: 0)
        flexerListB.append(GenerateFlexers.generate(size: 1))
        
        let flexer_rule_a = SkeletonLinkageRule_Flexers(flexers: flexerListA, layoutPriority: .required)
        let flexer_rule_b = SkeletonLinkageRule_Flexers(flexers: flexerListB, layoutPriority: .required)
        
        let node_a = GenerateNodes.generate(flexers: flexerListA)
        node_a.currentSize = 0
        node_a.childrenSize = 0
        
        let node_b = GenerateNodes.generate(flexers: flexerListB)
        ...
        let book = SkeletonBook(rows: [row_a],
                                nodeRules: [node_rule],
                                flexerRules: [flexer_rule_a, flexer_rule_b],
                                pieceRules: [])
    ...
    @MainActor @Test func grow_large_flexer_grow_no_gaps_10_exact_fit_utility() {
    ...
    @MainActor @Test func grow_large_flexer_grow_no_gaps_10_miss_by_1_a() {
        
        var flexerListA = GenerateFlexers.generate_n(10, size: 0)
        flexerListA.append(GenerateFlexers.generate(size: 1))
        
        var flexerListB = GenerateFlexers.generate_n(11, size: 0)
        flexerListB.append(GenerateFlexers.generate(size: 1))
    ...
    @MainActor @Test func grow_large_flexer_grow_no_gaps_10_miss_by_1_a_utility() {
        
        let node_a = GenerateNodes.generate(flexers: [])
        node_a.currentSize = 0
        node_a.childrenSize = 0
        node_a.requestedGrowthFromChildren = 10
        
        let node_b = GenerateNodes.generate(flexers: [])
        node_b.currentSize = 0
        node_b.childrenSize = 0
    ...
    @MainActor @Test func grow_large_flexer_grow_no_gaps_10_miss_by_1_b() {
        
        var flexerListA = GenerateFlexers.generate_n(11, size: 0)
        flexerListA.append(GenerateFlexers.generate(size: 1))
    ...
    @MainActor @Test func grow_large_flexer_grow_no_gaps_10_miss_by_1_b_utility() {
    ...
    @MainActor @Test func grow_large_flexer_grow_no_gaps_10_miss_by_1_c() {
    ...
    @MainActor @Test func grow_large_flexer_grow_no_gaps_10_miss_by_1_c_utility() {
    ...
    @MainActor @Test func grow_3_nodes_test_2_2_weave_a() {
        ...
        let flexer_a_1 = GenerateFlexers.generate(size: 2)
        let flexer_a_2 = GenerateFlexers.generate(size: 2)
        let flexer_b_1 = GenerateFlexers.generate(size: 2)
        let flexer_b_2 = GenerateFlexers.generate(size: 2)
        let flexer_grow_1 = GenerateFlexers.generate(size: 4)
        let flexer_grow_2 = GenerateFlexers.generate(size: 4)
        let flexer_rule_a = SkeletonLinkageRule_Flexers(flexers: [flexer_a_1, flexer_b_1, flexer_grow_1], layoutPriority: .required)
        let flexer_rule_b = SkeletonLinkageRule_Flexers(flexers: [flexer_a_2, flexer_b_2, flexer_grow_2], layoutPriority: .required)
        let node_a = GenerateNodes.generate(flexers: [flexer_a_1, flexer_a_2, flexer_grow_1])
        let node_b = GenerateNodes.generate(flexers: [flexer_b_1, flexer_b_2, flexer_grow_2])
        let section_a = GenerateSections.generate(nodes: [node_a, node_b])
        let row_a = GenerateRows.generate(sections: [section_a])
        let node_rule = SkeletonLinkageRule_Nodes(nodes: [node_a, node_b],
                                                  layoutPriority: .required)
        ...
        #expect(flexer_a_1.currentSize == 2)
        #expect(flexer_a_2.currentSize == 2)
        #expect(flexer_b_1.currentSize == 2)
        #expect(flexer_b_2.currentSize == 2)
        #expect(flexer_grow_1.currentSize == 4)
        #expect(flexer_grow_2.currentSize == 4)
        ...
        #expect(flexer_a_1.currentSize == 3)
        #expect(flexer_a_2.currentSize == 3)
        #expect(flexer_b_1.currentSize == 3)
        #expect(flexer_b_2.currentSize == 3)
        #expect(flexer_grow_1.currentSize == 4)
        #expect(flexer_grow_2.currentSize == 4)
        #expect(node_a.currentSize == 10)
        #expect(node_b.currentSize == 10)
        ...
        #expect(flexer_a_1.currentSize == 4)
        #expect(flexer_a_2.currentSize == 4)
        #expect(flexer_b_1.currentSize == 4)
        #expect(flexer_b_2.currentSize == 4)
        #expect(flexer_grow_1.currentSize == 4)
        #expect(flexer_grow_2.currentSize == 4)
    ...
    @MainActor @Test func grow_3_nodes_test_case_7_7_7_equal_target() {
        ...
        let nr_a = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 4, currentSize: 4, growCount: 3, amount: 1)
        let nr_b = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 2, currentSize: 2, growCount: 5, amount: 1)
        let nr_c = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 6, currentSize: 6, growCount: 1, amount: 1)
        ...
        let book = SkeletonBook(rows: [row_a],
                                nodeRules: [node_rule],
                                flexerRules: [nr_a.1, nr_b.1, nr_c.1],
                                pieceRules: [])
    ...
    @MainActor @Test func grow_5_nodes_mixed_ownership_a() {
        ...
        let nr_a = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 10, currentSize: 10, growCount: 10, amount: 1)
        let node_a = nr_a.0
        
        let nr_b = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 10, currentSize: 10, growCount: 10, amount: 1)
        let node_b = nr_b.0
        
        let nr_c = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 5, currentSize: 10, growCount: 10, amount: 1)
        let node_c = nr_c.0
        
        let nr_d = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 5, currentSize: 5, growCount: 15, amount: 1)
        let node_d = nr_d.0
        
        let nr_e = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 5, currentSize: 10, growCount: 10, amount: 1)
        let node_e = nr_e.0
        ...
        let book = SkeletonBook(rows: rows,
                                ...
                                flexerRules: [nr_a.1, nr_b.1, nr_c.1, nr_d.1, nr_e.1],
                                pieceRules: [])
    ...
    @MainActor @Test func grow_5_nodes_mixed_ownership_b() {
        ...
        let nr_a = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 10, currentSize: 15, growCount: 5, amount: 1)
        let node_a = nr_a.0
        
        let nr_b = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 0, currentSize: 5, growCount: 15, amount: 1)
        let node_b = nr_b.0
        
        let nr_c = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 0, currentSize: 10, growCount: 15, amount: 1)
        let node_c = nr_c.0
        
        let nr_d = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 10, currentSize: 15, growCount: 30, amount: 1)
        let node_d = nr_d.0
        
        let nr_e = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 0, currentSize: 5, growCount: 20, amount: 1)
        let node_e = nr_e.0
    ...
    @Test @MainActor func testFailingRandomCase_A() {
        ...
        let nnod_a = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 1, currentSize: 2, growCount: 2, amount: 1)
        let nod_a = nnod_a.0
        let nnod_b = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: 2, currentSize: 2, growCount: 0, amount: 1)
        let nod_b = nnod_b.0
        ...
        var flexerRules = [SkeletonLinkageRule_Flexers]()
        flexerRules.append(nnod_a.1)
        flexerRules.append(nnod_b.1)
        ...
        let pulseResultA = SmartLayoutExpanderPulse.step_c_apply_flexers()
    ...
    @MainActor @Test func grow_up_tp_4_nodes_multi_pulse_check_a() {
        ...
        for _ in 0..<1000 {
            ...
            for partition in partitions {
                ...
                var flexer_rules = [SkeletonLinkageRule_Flexers]()
                ...
                for items in partition {
                    ...
                    for item in items {
                        let node = GenerateNodes.generate_growing_flexers_group_rules(childrenSize: item.childrenSize,
                                                                         currentSize: item.currentSize,
                                                                         growCount: item.wtg,
                                                                         amount: 1)
                        subnodes.append(node.0)
                        flexer_rules.append(node.1)
                    }
