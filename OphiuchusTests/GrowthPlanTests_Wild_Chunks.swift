...
struct GrowthPlanTests_Wild_Chunks {
    ...
    func validate(layoutRows: [SkeletonRow],
                  layoutChunks: [any SkeletonChunkConforming],
                  rowGrowthPlansList: [RowGrowthPlans]) -> Bool {
        
        // We keep in mind, this is for *nodes*
        
        
        for layoutChunk in layoutChunks {
            let chunkRow = layoutChunk.row!
            var growthPlanExists = false
            for _rowGrowthPlans in rowGrowthPlansList {
                if _rowGrowthPlans.layoutRow === chunkRow {
                    growthPlanExists = true
                }
            }
            if !growthPlanExists {
                print("Failed! Chunk \(chunkRow.id) from row \(chunkRow.id) was *not* found in the growth plans!")
                return false
            }
        }
        ...
        for _ in 0..<4096 {
            ...
            for _ in 0..<sectionCount {
                ...
                for _ in 0..<nodeCount {
                    //let node = GenerateChunks.generate_
                    
                    let chunkCount = Int.random(in: 1..<4)
                ...
                let layoutSection = GenerateSections.generate_section(skeleton_nodes: skeleton_nodes, gap: section_gap)
